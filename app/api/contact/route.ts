...
// Maksimum yeniden deneme (retry) sayısı ve başlangıç gecikme süresi
const MAX_RETRIES = 3;
const RETRY_DELAY_MS = 1000;

async function sendEmailWithRetry(payload: object, apiKey: string, retries = MAX_RETRIES): Promise<Response> {
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            // Eğer sunucu hata döndüyse ve son deneme değilse tekrar dene
            if (attempt === retries) return response;
        } catch (error) {
            if (attempt === retries) throw error;
        }

        // Üstel artan gecikme (backoff) ile bekle
...
export async function POST(request: Request) {
    ...
    try {
        ...
        const contactFrom = process.env.CONTACT_FROM_EMAIL || "info@plastictopaper.org";
        ...
        const emailPayload = {
            from: contactFrom,
            to: [contactTo],
            subject: `PTP project enquiry — ${body.company}`,
            text: `Yeni Proje Başvurusu / Project Enquiry:\n\n` +
                `Ad Soyad / Name: ${body.name}\n` +
                `Şirket / Company: ${body.company}\n` +
                `E-posta / Email: ${body.email}\n` +
                `Ülke / Country: ${body.country}\n` +
